...
// 允许记录时间
@property(nonatomic,assign)CGFloat requireTime;
/* 各视图控件结束 */
...
-(void)startTime
{
    ...
    self.threshold.frame=CGRectMake(x,
                                    self.threshold.frame.origin.y,
                                    self.threshold.frame.size.width,
                                    self.threshold.frame.size.height);
    if (!self.timer) {
